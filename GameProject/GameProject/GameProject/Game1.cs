...
namespace GameProject
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            healthString = GameConstants.HEALTH_PREFIX+burger.Healt;
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (var i = 0; i < bears.Count; i++)
            {
                for (var j = i + 1; j < bears.Count; j++)
                {
                    ...
                    {
                        CollisionResolutionInfo colision = CollisionUtils.CheckCollision(gameTime.ElapsedGameTime.Milliseconds, 
                        ...
                        {
                            teddyBounce.Play();
                            if (colision.FirstOutOfBounds)
                            {
                                bears[i].Active = false;
                            }
                            else
                            {
                                bears[i].DrawRectangle = colision.FirstDrawRectangle;
                                bears[i].Velocity = colision.FirstVelocity;
                            }
                            if (colision.SecondOutOfBounds)
                            {
                                bears[j].Active = false;
                            }
                            else
                            {
                                bears[j].DrawRectangle = colision.SecondDrawRectangle;
                                bears[j].Velocity = colision.SecondVelocity;
                            }
                        }
                    }
                }
            ...
            foreach (TeddyBear bear in bears)
            {
                ...
                {
                    burger.Healt = burger.Healt - GameConstants.BEAR_DAMAGE;
                    healthString = GameConstants.HEALTH_PREFIX + burger.Healt;
                    CheckBurgerKill();
                    bear.Active = false;
            ...
            foreach (Projectile projectile in projectiles)
            {
                ...
                {
                    burger.Healt = burger.Healt - GameConstants.TEDDY_BEAR_PROJECTILE_DAMAGE;
                    healthString = GameConstants.HEALTH_PREFIX + burger.Healt;
                    CheckBurgerKill();
        ...
        private void SpawnBear()
        {
            ...
            List<Rectangle> otherRectangles = this.GetCollisionRectangles();
            while (!CollisionUtils.IsCollisionFree(newBear.DrawRectangle, otherRectangles))
            {
